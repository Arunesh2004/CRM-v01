...
export class RestoreCoordinator {
  ...
  async requestRestore(tenantId: string, archiveLocation: string, requestorUserId: string): Promise<string> {
    ...
    const latestJob = await prismaAdmin.recoveryJob.findFirst({
      where: { tenantId, status: 'IN_PROGRESS' },
      orderBy: { createdAt: 'desc' }
    });

    const job = await prismaAdmin.recoveryJob.findUnique({
      where: { id: jobIdParam }
    });
    ...
    const snapshot = await prismaAdmin.recoverySnapshot.findUnique({
      where: { id: job.snapshotId, status: 'ACTIVE' }
    });
