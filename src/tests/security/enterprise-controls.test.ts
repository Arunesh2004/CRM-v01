...
describe('Phase 10.3 - Enterprise Controls - Adversarial Security Tests', () => {
  ...
  beforeAll(async () => {
    // Setup 2 Tenants
    const t1 = await prisma.tenant.create({ data: { name: 'Ent Test T1', status: 'ACTIVE' } });
    const t2 = await prisma.tenant.create({ data: { name: 'Ent Test T2', status: 'ACTIVE' } });
    tenant1Id = t1.id;
    tenant2Id = t2.id;

    // Setup Users
    const u1 = await prisma.user.create({ data: { tenantId: tenant1Id, email: 'admin@t1.ent', clerkId: 'c_ent_admin', status: 'ACTIVE', firstName: 'A', lastName: 'A' } });
    const u2 = await prisma.user.create({ data: { tenantId: tenant1Id, email: 'hacker@t1.ent', clerkId: 'c_ent_hacker', status: 'ACTIVE', firstName: 'H', lastName: 'H' } });
    const u3 = await prisma.user.create({ data: { tenantId: tenant1Id, email: 'approver@t1.ent', clerkId: 'c_ent_approver', status: 'ACTIVE', firstName: 'Ap', lastName: 'Ap' } });
    ...
    const adminRole = await prisma.role.create({ data: { tenantId: tenant1Id, name: 'TENANT_ADMIN' } });
    await prisma.userRole.create({ data: { userId: adminId, roleId: adminRole.id } });
  });

  afterAll(async () => {
    // await prisma.auditLog.deleteMany({ where: { tenantId: { in: [tenant1Id, tenant2Id] } } });
    await prisma.securityEvent.deleteMany({ where: { tenantId: { in: [tenant1Id, tenant2Id] } } });
    await prisma.aBACPolicy.deleteMany({ where: { tenantId: { in: [tenant1Id, tenant2Id] } } });
    await prisma.fieldSecurityPolicy.deleteMany({ where: { tenantId: { in: [tenant1Id, tenant2Id] } } });
    await prisma.aIExecution.deleteMany({ where: { tenantId: { in: [tenant1Id, tenant2Id] } } });
    await prisma.approvalStep.deleteMany({ where: { tenantId: { in: [tenant1Id, tenant2Id] } } });
    await prisma.approvalRequest.deleteMany({ where: { tenantId: { in: [tenant1Id, tenant2Id] } } });
    await prisma.userRole.deleteMany({ where: { user: { tenantId: { in: [tenant1Id, tenant2Id] } } } });
    await prisma.role.deleteMany({ where: { tenantId: { in: [tenant1Id, tenant2Id] } } });
    await prisma.user.deleteMany({ where: { tenantId: { in: [tenant1Id, tenant2Id] } } });
    await prisma.aITool.deleteMany({ where: { name: 'admin_tool_ent_test' } });
  ...
  describe('APPROVALS', () => {
    ...
    it('13. Unauthorized approver', async () => {
      const randUser = await prisma.user.create({ data: { tenantId: tenant1Id, email: 'rand@t1.ent', clerkId: 'c_ent_rand', status: 'ACTIVE', firstName: 'R', lastName: 'R' } });
      await expect(
