...
export function EditLeadForm({ lead }: { lead: any }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await updateLeadAction(data);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0 animate-in fade-in duration-200">
          ...
          >
            <div className="px-6 py-5 border-b border-white/[.06] flex items-center justify-between" style={{ background: 'rgba(13,19,38,.6)' }}>
              <div>
                <h2 className="text-xl font-display font-bold text-white tracking-tight">Edit Lead</h2>
                <p className="text-sm text-[#8891B0] mt-1">Update prospect details.</p>
              </div>
              <button 
                onClick={() => setIsOpen(false)}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div>
                <label className="block text-sm font-medium text-[#8891B0] mb-1.5">Lead Name <span className="text-rose-400">*</span></label>
                <input 
                  required 
                  name="name" 
                  type="text" 
              ...
              <div>
                <label className="block text-sm font-medium text-[#8891B0] mb-1.5">Company <span className="text-rose-400">*</span></label>
                <input 
                  required
                  name="company" 
                  type="text" 
              ...
              <div>
                ...
                >
                  <option value="NEW" className="bg-[#0D1326]">New</option>
                  <option value="CONTACTED" className="bg-[#0D1326]">Contacted</option>
                  <option value="QUALIFIED" className="bg-[#0D1326]">Qualified</option>
                  <option value="LOST" className="bg-[#0D1326]">Lost</option>
                </select>
              </div>
