...
export function useMediaMTXWebRTC(cameraId: string) {
  ...
  const connect = useCallback(
    async (isReconnect: boolean = false) => {
      ...
      // Clear old token on new attempt
  ...
  const handleFailure = useCallback(
    (attemptId: symbol, customError?: WebRTCErrorType) => {
      if (attemptIdRef.current !== attemptId) return;
      ...
      if (
        customError === "AUTH_ERROR" ||
        customError === "MISSING_CREDENTIALS"
      ) {
        setState((prev) => ({
          ...prev,
          status: "error",
      ...
      if (retriesRef.current > MAX_RETRIES) {
        setState((prev) => ({
          ...prev,
          status: "error",
          errorType: customError || "NETWORK_ERROR",
        }));
        return;
      }
