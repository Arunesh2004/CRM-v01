...
import { getDealsAction, getPipelinesAction, getDealAnalyticsAction, seedDefaultPipelineAction } from '@/modules/crm/actions/deal.actions';
...
export default async function DealsPage({ searchParams }: { searchParams: { [key: string]: string | string[] | undefined } }) {
  ...
  const pipelines: any[] = pipelineRes.success && pipelineRes.data ? pipelineRes.data : [];
  
  if (pipelines.length === 0) {
    return <div>Error loading pipelines</div>;
  }

  const selectedPipelineId = typeof searchParams.pipeline === 'string' ? searchParams.pipeline : pipelines[0]?.id;
  const pipeline = pipelines.find((p: any) => p.id === selectedPipelineId) || pipelines[0];
    ...
    <div className="flex flex-col gap-6 h-[calc(100vh-6rem)]">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage opportunities and revenue forecasts.</p>
        </div>
        <div className="flex items-center gap-2">
          {/* Pipeline Selector could go here */}
          <select className="border rounded-md px-3 py-2 text-sm bg-background" defaultValue={pipeline.id}>
            {pipelines.map((p: any) => (
      ...
      {metrics && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">${metrics.weightedPipelineValue.toLocaleString()}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">${metrics.wonRevenue.toLocaleString()}</div>
            </CardContent>
          </Card>
        </div>
      )}
