...
export function EmployeeInviteForm({ departments = [] }: { departments?: any[] }) {
    ...
    <form action={action} className="space-y-4">
      ...
      <div>
        ...
        >
          ...
          <option value="DEPARTMENT_HEAD" style={{ background: '#0D1326', color: '#E7EAF5' }}>Department Head</option>
        </select>
      </div>
      <div>
        <label className="block text-sm font-medium text-[#8891B0] mb-1.5">Department</label>
        <select 
          name="departmentId" 
          className="w-full text-sm transition-all focus:ring-2 focus:ring-[#7C5CFC] focus:border-transparent appearance-none cursor-pointer"
          style={{
        ...
        >
          <option value="" style={{ background: '#0D1326', color: '#E7EAF5' }}>Select Department...</option>
          {departments.map((d: any) => (
            <option key={d.id} value={d.id} style={{ background: '#0D1326', color: '#E7EAF5' }}>{d.name}</option>
          ))}
        </select>
      </div>
